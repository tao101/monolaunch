...
export async function setupSupabase(projectPath) {
...
export async function setupShadcn(projectPath, addAllComponents = true) {
...
export function createEnvFile(projectPath, isExpoApp = false) {
    if (isExpoApp) {
...
export function createSupabaseClient(projectPath, isExpoApp = false) {
...
export function createNextjsMiddleware(projectPath) {
...
export function createCoolifyConfig(projectPath, appName) {
...
export function createPnpmWorkspace(projectPath) {
...
export function createSharedPackage(projectPath) {
...
export function updateSupabaseConfig(projectPath) {
    ...
    try {
        ...
        // Update site_url
        configContent = configContent.replace(/site_url = "[^"]*"/, 'site_url = "http://localhost:3000"');
        ...
        if (redirectUrlsPattern.test(configContent)) {
            ...
            configContent = configContent.replace(/(\[auth\][^[]*site_url = "[^"]*")/, `$1\n${newRedirectUrls}`);
        }
...
uri = "pg-functions://postgres/public/custom_access_token_hook"`;
        if (configContent.includes('[auth.hook.custom_access_token]')) {
            ...
            configContent = configContent.replace(/\[auth\.hook\.custom_access_token\][^[]*(?=\[|$)/, hookSection + '\n');
        }
        else {
            // Add hook section at the end of [auth] section
            configContent = configContent.replace(/(\[auth\][^[]*)/, `$1${hookSection}\n`);
        }
...
export function setupSupabaseTypes(projectPath, isExpoApp = false) {
...
export function updatePackageJsonScripts(projectPath, isExpoApp = false, isMonorepoRoot = false) {
...
export function createProjectReadme(projectPath, projectName, isMonorepo, templateType) {
...
export function createProjectClaudeMd(projectPath, projectName, isMonorepo, templateType) {
...
export function setupExpoRouter(projectPath) {
    try {
...
export function setupReactNativeReusables(projectPath) {
    try {
...
export function setupLegendState(projectPath, isExpoApp = false) {
    try {
